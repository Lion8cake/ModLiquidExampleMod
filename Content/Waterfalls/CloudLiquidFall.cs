		...
		public override bool PreDraw(WaterfallData currentWaterfallData, int i, int j, SpriteBatch spriteBatch)
		{
			...
			for (int step = 0; step < maxDist; step++)
			{
				...
				if (topSlope && dir != prevDirX)
				{
					...
					{
						Main.instance.waterfallManager.DrawWaterfall(Slot, x, y, alpha, new Vector2(x * 16, y * 16 + 16 - num39) - Main.screenPosition, new Rectangle(frame, 24, 32, 16 - liquidAmount - num39), lightingColor, (SpriteEffects)0 | (SpriteEffects)2);
					}
				}
				...
				if (prevHasSlope != 0 && newX == 0 && newY == -1)
				{
					...
					{
						Main.instance.waterfallManager.DrawWaterfall(Slot, x, y, alpha, new Vector2(x * 16, y * 16 + yOff + 8) - Main.screenPosition, new Rectangle(frame, 0, 16, 16 - liquidAmount - 8), lightingColor, (SpriteEffects)0 | (SpriteEffects)2);
					}
				}
				if (yOff == -8 && prevNewY == 1 && prevHasSlope == 0)
				{
					if (dirX == -1)
					{
						if (prevType != Slot)
						{
							Main.instance.waterfallManager.DrawWaterfall(prevType, x, y, alpha, new Vector2(x * 16, y * 16) - Main.screenPosition, new Rectangle(frame, 24, 32, 8), prevlightColor, (SpriteEffects)0 | (SpriteEffects)2);
						}
						else
						{
							Main.instance.waterfallManager.DrawWaterfall(Slot, x, y, alpha, new Vector2(x * 16, y * 16) - Main.screenPosition, new Rectangle(frame, 24, 32, 8), lightingColor, (SpriteEffects)0 | (SpriteEffects)2);
						}
					}
				...
				if (hasSlope != 0 && prevNewX == 0)
				{
					...
					{
						Main.instance.waterfallManager.DrawWaterfall(prevType, x, y, alpha, new Vector2(x * 16, y * 16) - Main.screenPosition, new Rectangle(frame, 24, 32, 16 - liquidAmount), prevlightColor, (SpriteEffects)0 | (SpriteEffects)2);
					}
					else
					{
						Main.instance.waterfallManager.DrawWaterfall(Slot, x, y, alpha, new Vector2(x * 16, y * 16) - Main.screenPosition, new Rectangle(frame, 24, 32, 16 - liquidAmount), lightingColor, (SpriteEffects)0 | (SpriteEffects)2);
					}
				}
				if (newY == -1 && hasSlope == 0 && prevHasSlope == 0)
				{
					if (dir == -1)
					{
						if (prevNewY == 0)
						{
							Main.instance.waterfallManager.DrawWaterfall(Slot, x, y, alpha, new Vector2(x * 16, y * 16 + yOff) - Main.screenPosition, new Rectangle(frame, 0, 16, 16 - liquidAmount), lightingColor, (SpriteEffects)0 | (SpriteEffects)2);
							Main.instance.waterfallManager.DrawWaterfall(Slot, x, y, alpha, new Vector2(x * 16, y * 16 - 16) - Main.screenPosition, new Rectangle(frame, 24, 32, 16 - liquidAmount), lightingColor, (SpriteEffects)0 | (SpriteEffects)2);
						}
						else if (prevType != Slot)
						{
							Main.instance.waterfallManager.DrawWaterfall(prevType, x, y, alpha, new Vector2(x * 16, y * 16 - 16) - Main.screenPosition, new Rectangle(frame, 24, 32, 16 - liquidAmount), prevlightColor, (SpriteEffects)0 | (SpriteEffects)2);
						}
						else
						{
							Main.instance.waterfallManager.DrawWaterfall(Slot, x, y, alpha, new Vector2(x * 16, y * 16 - 16) - Main.screenPosition, new Rectangle(frame, 24, 32, 16 - liquidAmount), lightingColor, (SpriteEffects)0 | (SpriteEffects)2);
					...
					switch (newX)
					{
						...
						case -1:
							...
							else
							{
								...
								Main.instance.waterfallManager.DrawWaterfall(Slot, x, y, alpha, new Vector2(x * 16, y * 16 + yOff) - Main.screenPosition, new Rectangle(16 + frame, 0, 16, height), lightingColor, (SpriteEffects)0 | (SpriteEffects)2);
							}
							break;
						case 0:
							if (newY == 0)
							{
								if (Main.tile[x, y].LiquidAmount <= 0 || Main.tile[x, y].IsHalfBlock)
								{
									Main.instance.waterfallManager.DrawWaterfall(Slot, x, y, alpha, new Vector2(x * 16, y * 16 + yOff) - Main.screenPosition, new Rectangle(16 + frame, 0, 16, 16), lightingColor, (SpriteEffects)0 | (SpriteEffects)2);
								}
								step = 1000;
							}
							break;
